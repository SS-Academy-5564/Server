...
public class PasswordResetControllerTests
{
    ...
    public async Task VerifyCodeAsync_OnSuccess_ReturnsOkWithToken()
    {
        ...
        _verifyHandlerMock
            .Setup(x => x.HandleAsync(It.Is<VerifyPasswordResetCodeCommand>(c => c.Email == request.Email && c.Code == request.Code), It.IsAny<CancellationToken>()))
    ...
    public async Task VerifyCodeAsync_OnFailure_ReturnsBadRequest()
    {
        ...
        _verifyHandlerMock
            .Setup(x => x.HandleAsync(It.IsAny<VerifyPasswordResetCodeCommand>(), It.IsAny<CancellationToken>()))
    ...
    public async Task ResetPasswordAsync_OnSuccess_ReturnsOk()
    {
        ...
        _resetHandlerMock
            .Setup(x => x.HandleAsync(It.Is<ResetPasswordCommand>(c => c.ResetToken == request.ResetToken && c.NewPassword == request.NewPassword), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Ok());
